...
def train_model(model, key, input_locations, output_locations, n_holdout, minimum_epochs, maximum_epochs_no_improve, maximum_epochs, batch_size, train_val_split):
    ...
    train_input_locations = input_locations[train_indices]
    train_output_locations = output_locations[train_indices]

    val_input_locations = input_locations[val_indices]
    val_output_locations = output_locations[val_indices]

    test_input_locations = input_locations[test_indices]
    ...
    for epoch in range(maximum_epochs):
        ...
        if epochs_no_improve > maximum_epochs_no_improve:
            ...
            test_inputs_npy = np.array(test_input_locations)
            np.save("../data/test_prediction_mu_MLP.npy", test_prediction_mu_npy)
            np.save("../data/test_prediction_sigma_sq_MLP.npy", test_prediction_sigma_sq_npy)
            np.save("../data/test_target_MLP.npy", test_target_npy)
            np.save("../data/test_inputs_MLP.npy", test_inputs_npy)

            np.save("../data/train_losses_MLP.npy", train_losses)
            np.save("../data/val_losses_MLP.npy", val_losses)
            ...
            val_inputs_npy = np.array(val_input_locations)
            np.save("../data/val_prediction_mu_MLP.npy", val_prediction_mu_npy)
            np.save("../data/val_prediction_sigma_sq_MLP.npy", val_prediction_sigma_sq_npy)
            np.save("../data/val_target_MLP.npy", val_target_npy)
            np.save("../data/val_inputs_MLP.npy", val_inputs_npy)
            break
